	...
	return (
		<div className="mx-auto w-full max-w-5xl space-y-8 py-12 px-6">
			<PublicProfileHeader
				...
				avatarFallback={player.displayName.slice(0, 2).toUpperCase()}
					...
					<>
						{user ? (
							<Button asChild size="sm" variant="outline">
								<Link href={appRoutes.profile}>Open your workspace</Link>
							</Button>
						) : (
			...
			{player.bio && <p className="max-w-[68ch] text-sm text-muted-foreground">{player.bio}</p>}
			...
			<div className="grid gap-8 lg:grid-cols-3">
				<div className="space-y-8 lg:col-span-2">
					...
					<PublicPageSection
						title="Open listings"
						description="Availability and recruiting listings published directly by this player."
					>
				...
				<aside className="space-y-6">
					<PublicPageSection title="Player details">
						<dl className="space-y-3 border p-4">
							{player.battletag && (
								<div>
									...
									<dd className="mt-1 text-sm font-semibold">{player.battletag}</dd>
								</div>
							)}
							{player.scrimStats && (
								<>
									<div>
										...
										<dd className="mt-1 text-sm font-semibold">{player.scrimStats.scrimsPlayed}</dd>
									</div>
									<div>
										<dt className="text-[10px] font-medium uppercase tracking-wide text-muted-foreground">
											Record
