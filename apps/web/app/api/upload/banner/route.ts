...
const MAX_BYTES = 4 * 1024 * 1024; // 4 MB — banners are wider/larger
const BUCKET = process.env.S3_BUCKET_BANNERS ?? "banners";
...
export async function POST(request: Request) {
	...
	if (file.size > MAX_BYTES) {
		return NextResponse.json({ error: "File must be smaller than 4 MB" }, { status: 400 });
	}
	...
	if (userRow?.bannerUrl) {
		const oldKey = keyFromUrl(userRow.bannerUrl, BUCKET);
		if (oldKey) {
