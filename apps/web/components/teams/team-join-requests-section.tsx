...
import { respondToTeamJoinRequestAction } from "@/app/dashboard/teams/[teamId]/actions/invites";
import { Avatar, AvatarFallback, AvatarImage } from "@/components/ui/avatar";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { Spinner } from "@/components/ui/spinner";
import { useFormAction } from "@/hooks/use-form-action";
import type { TeamJoinRequestSummary } from "@/lib/data/team";
...
function RequestActions({ teamId, requestId }: { teamId: string; requestId: string }) {
	const { submit, isPending } = useFormAction(respondToTeamJoinRequestAction, {
...
export function TeamJoinRequestsSection({ teamId, requests }: TeamJoinRequestsSectionProps) {
		...
		<div className="space-y-2">
			{requests.map((request) => (
				<div key={request.id} className="flex items-center gap-3 border px-4 py-3">
					...
					<RequestActions teamId={teamId} requestId={request.id} />
				</div>
			))}
		</div>
	);
