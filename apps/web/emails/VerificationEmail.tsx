...
export const VerificationEmail = ({ code, title, message, actionText }: VerificationEmailProps) => {
	return (
		<Html>
			<Head />
			...
			<Tailwind>
				<Body className="bg-white font-sans text-gray-800 m-0 p-0">
					...
					<Container className="mx-auto p-6 max-w-145">
						<Heading className="text-2xl font-bold text-gray-900 m-0 mb-4 mt-8">{title}</Heading>

						<Text className="text-[14px] leading-6 mb-2">{message}</Text>
						<Text className="text-[14px] leading-6 mb-6">To continue, please {actionText}:</Text>
