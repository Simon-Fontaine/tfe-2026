...
Ubuntu 22.04 LTS, single VPS, behind the Cloudflare proxy. Cloudflare serves TLS to visitors; Caddy serves a Cloudflare Origin certificate on the origin.
...
These open `80`/`443` to the whole internet temporarily; tighten them to Cloudflare's IP
ranges once the proxy is on — see [Section 4](#4-dns--cloudflare). Caddy listens on `80`
only to 301-redirect stray plain-HTTP requests to `443`.
...
Point the A record (and AAAA if you have IPv6) for `scrimflow.com` to the VPS IP. Add a `www` record too (a `CNAME www → scrimflow.com`, or its own A/AAAA) — Caddy redirects `www.scrimflow.com` to the apex, but the hostname must still resolve and reach the origin for that redirect to be served.
...
Set both records to **Proxied (orange cloud)**, then set **SSL/TLS → Overview → Full (strict)**. Cloudflare terminates TLS for visitors and validates the origin certificate (Section 4.2) end-to-end.

### 4.2 Generate and install the origin certificate

Cloudflare terminates visitor TLS at its edge, so the origin only needs a certificate Cloudflare trusts. In the dashboard: **SSL/TLS → Origin Server → Create Certificate**. Accept the default (RSA, key generated by Cloudflare) and set the hostnames to `scrimflow.com` **and** `*.scrimflow.com` so the cert covers `www`. Cloudflare shows two blocks — the **Origin Certificate** and the **Private Key**. Install them into the cloned repo in [Section 5](#5-deploy).

Caddy serves them via the `tls` directive in [`deploy/caddy/Caddyfile`](deploy/caddy/Caddyfile).

### 4.3 Why the origin must be locked to Cloudflare
...
### 4.4 Restrict 80/443 to Cloudflare
...
To have Caddy's access logs show the real visitor IP, add `trusted_proxies static <same CIDRs>` to the apex site block in `deploy/caddy/Caddyfile`.
...
Fill every `change_me_*` value; [`.env.production.example`](.env.production.example) documents each variable inline. Key secrets:
...
Then install the Cloudflare Origin certificate from [Section 4.2](#42-generate-and-install-the-origin-certificate):

```bash
mkdir -p deploy/caddy/certs
nano deploy/caddy/certs/origin.pem      # paste the Origin Certificate
nano deploy/caddy/certs/origin-key.pem  # paste the Private Key
chmod 600 deploy/caddy/certs/origin-key.pem
```

These files are gitignored (`*.pem`/`*.key`) and never committed.
...
This builds the images, runs migrations via `db-migrate`, initialises MinIO buckets via `storage-init`, and starts all services. Caddy serves the installed origin certificate immediately.
