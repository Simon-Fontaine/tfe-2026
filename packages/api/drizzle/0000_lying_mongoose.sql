...
CREATE TYPE "public"."moderation_action_type" AS ENUM('warn', 'suspend', 'restore', 'hide', 'unhide', 'remove', 'require_verification', 'clear_verification', 'escalate');--> statement-breakpoint
...
CREATE TYPE "public"."recruitment_listing_status" AS ENUM('open', 'paused', 'closed', 'fulfilled', 'expired');--> statement-breakpoint
...
CREATE TYPE "public"."verification_action" AS ENUM('email_change', 'account_deletion', 'password_change', 'two_factor_disable', 'passkey_disable', 'security_key_disable', 'recovery_code_regenerate', 'organization_lifecycle_delete', 'team_lifecycle_delete');--> statement-breakpoint
...
CREATE TABLE "user" (
	...
	"requires_reverification" boolean DEFAULT false NOT NULL,
...
CREATE TABLE "organization" (
	...
	"is_moderation_suspended" boolean DEFAULT false NOT NULL,
...
CREATE TABLE "scrim" (
	...
	"dispute_responded_at" timestamp with time zone,
	"dispute_responded_by_user_id" uuid,
	"created_by_user_id" uuid,
	"message" text,
	"home_team_name_snapshot" text,
	"home_team_tag_snapshot" text,
	"away_team_name_snapshot" text,
	"away_team_tag_snapshot" text,
	"created_at" timestamp DEFAULT now() NOT NULL,
	"updated_at" timestamp DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
CREATE TABLE "team_rating_event" (
	...
	"algorithm_version" text DEFAULT 'glicko-1.0' NOT NULL,
	"created_at" timestamp DEFAULT now() NOT NULL
);
--> statement-breakpoint
...
ALTER TABLE "scrim" ADD CONSTRAINT "scrim_dispute_responded_by_user_id_user_id_fk" FOREIGN KEY ("dispute_responded_by_user_id") REFERENCES "public"."user"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "user_report" ADD CONSTRAINT "user_report_assigned_moderator_id_user_id_fk" FOREIGN KEY ("assigned_moderator_id") REFERENCES "public"."user"("id") ON DELETE set null ON UPDATE no action;--> statement-breakpoint
...
CREATE INDEX "domain_audit_event_created_idx" ON "domain_audit_event" USING btree ("created_at");--> statement-breakpoint
...
CREATE UNIQUE INDEX "notification_active_dedup_idx" ON "notification" USING btree ("user_id","type","reference_id") WHERE "notification"."is_read" = false;--> statement-breakpoint
...
CREATE UNIQUE INDEX "organization_name_unique_idx" ON "organization" USING btree (lower("name"));--> statement-breakpoint
...
CREATE INDEX "ownership_workflow_event_actor_idx" ON "ownership_workflow_event" USING btree ("actor_user_id");--> statement-breakpoint
...
CREATE INDEX "scrim_neg_rev_actor_user_idx" ON "scrim_negotiation_revision" USING btree ("actor_user_id");--> statement-breakpoint
...
CREATE UNIQUE INDEX "scrim_active_pair_unique_idx" ON "scrim" USING btree (LEAST("home_team_id", "away_team_id"),GREATEST("home_team_id", "away_team_id")) WHERE "scrim"."status" IN ('pending', 'accepted', 'scheduled', 'in_progress') AND "scrim"."away_team_id" IS NOT NULL;--> statement-breakpoint
...
CREATE UNIQUE INDEX "team_org_active_tag_unique_idx" ON "team" USING btree ("organization_id",upper("tag")) WHERE "team"."is_archived" = false;--> statement-breakpoint
...
CREATE INDEX "update_post_author_idx" ON "update_post" USING btree ("author_user_id","created_at");--> statement-breakpoint
